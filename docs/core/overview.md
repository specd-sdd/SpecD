...
The `infrastructure/` layer is internal — it is not exported from `@specd/core` and you should not import from it directly. Infrastructure adapters are implementation details of the package and may change without notice.

## Public exports

`@specd/core` exports from two paths:

```typescript
import { ... } from '@specd/core'       // domain + application
```

Everything exported is either a domain type (entity, value object, error) or an application type (use case class, port interface, error).
