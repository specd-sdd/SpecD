...
| `VALID_TRANSITIONS`   | const     | Protocol map of valid transitions per lifecycle state (includes drain edges). `availableTransitions` from `GetStatus` is check-derived. |
