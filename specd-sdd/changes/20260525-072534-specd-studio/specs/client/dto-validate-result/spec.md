...
Client-side type for **Dto Validate Result**, kept in parity with the matching `api:dto-*` spec so remote and embedded Studio render the same JSON the API emits.
...
- [`api:dto-validate-result`](../../api/dto-validate-result/spec.md) — mirror API DTO
