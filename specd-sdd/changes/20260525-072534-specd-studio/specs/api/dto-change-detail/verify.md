...
#### Scenario: Transitioned history entry includes state fields

- **GIVEN** kernel history contains a `transitioned` event
- **WHEN** `GET /changes/{name}` is serialized
- **THEN** that history element includes `from` and `to`
- **AND** `type`, `at`, and `by` are present

#### Scenario: Created history entry includes schema fields

- **GIVEN** kernel history contains a `created` event
