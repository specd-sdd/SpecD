# Spec Compliance Audit: improve-change-dependency-visibility
...
The change is ready for approval. The missing unit tests in `@specd/core` are mitigated by strong CLI-level verification, but it is recommended to add them before archiving the change to ensure long-term stability of the status projection logic.
