# Spec Compliance Report — validate-specs-result-cache

- **Mode:** change
- **Change:** validate-specs-result-cache
- **Timestamp:** 20260722-221711
- **State at audit:** verifying
- **Specs in scope:** core:validation-result-cache-port, core:storage, core:validate-specs
- **Globals checked:** default:\_global/architecture, default:\_global/testing

## Executive Summary

Implementation of the transparent ValidateSpecs result cache matches the change specs structurally (port, FS adapter, composition wiring, cascade, fingerprints). No behavioural blockers found that would fail scenario verification.

Compliance audit found **test-coverage gaps**, one **architecture barrel omission**, and one **verify.md inconsistency**. These are quality/conformance issues, not scenario failures of the implemented cache behaviour.

| Metric                                 | Count |
| -------------------------------------- | ----: |
| Requirements checked (new cache scope) |    28 |
| Implemented                            |    26 |
| Discrepancies                          |    10 |
| Missing tests (unique emphasis)        |    16 |

### Highest-risk gaps

1. Use-case soft-hit and miss+failure upsert paths are implemented but largely untested.
2. New port absent from curated `@specd/core/ports` barrel (`packages/core/src/ports.ts`).
3. Multi-workspace cache isolation untested.
4. Older `verify.md` factory scenario still omits `validationResultCaches` while the transparent-cache scenario requires it.

### Recommendation options for verify workflow

- **Update Specs** — fix verify factory scenario incompleteness; optionally clarify SpecRepository helper boundary / ports barrel expectation.
- **Fix Implementation** — add missing use-case/adapter tests; export `ValidationResultCache` from curated ports barrel.
- **Both** — preferred if tightening conformance before archive.
- **Proceed** — accept gaps and continue lifecycle (scenarios currently pass by inspection + existing suite).
