...
- **`id`** (`string`) — deterministic identifier computed from `filePath + kind + name + line + column`; the existing start-location identity remains unchanged.
...
- **`line`** (`number`) — 1-based start line of the complete syntactic construct.
- **`column`** (`number`) — 0-based start column of the complete syntactic construct.
- **`endLine`** (`number`) — 1-based end line of the complete syntactic construct.
- **`endColumn`** (`number`) — 0-based exclusive end column of the complete syntactic construct.
- **`selectionRange`** (`SourceRange`) — exact range of the declared name or equivalent addressable selection.
- **`comment`** (`string | undefined`) — the raw comment text.

`SourceRange` SHALL contain 1-based `startLine`/`endLine` and 0-based `startColumn`/exclusive `endColumn`. The complete construct range SHALL be non-empty, SHALL contain `selectionRange`, and SHALL be suitable for extracting the whole declaration or body from the indexed content. `line` and `column` remain aliases of the construct range start for compatibility and symbol-id construction.
...
The model SHALL represent persisted per-file/package indexing coverage and failure reasons, including indexed, excluded, unsupported adapter/capability, parse-failed, and partial states. Coverage facts SHALL include the indexed content hash needed to determine whether absence is trustworthy.
