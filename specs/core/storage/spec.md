...
"Cleaned hash" means: read the file content, apply the artifact type's `preHashCleanup` rules in order, then apply universal whitespace normalization (collapsing any whitespace sequence `\s+` into a single space `' '` and trimming leading and trailing whitespace), then compute SHA-256 of the result (the same normalization `ValidateArtifacts` applies before computing `validatedHash`). `FsChangeRepository` must have access to the schema's artifact types at load time so that it can retrieve the `preHashCleanup` array for each artifact type. If no `preHashCleanup` rules are defined for an artifact type, the content is still whitespace-normalized before hashing.
