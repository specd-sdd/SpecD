...
- [`core:change`](../change/spec.md) — defines the Change entity that stores `specIds` and `specDependsOn`
- [`core:change-manifest`](../change-manifest/spec.md) — defines manifest serialization of `specIds`
- [`core:spec-metadata`](../spec-metadata/spec.md) — defines `dependsOn` field that uses colon-qualified format
- [`core:compile-context`](../compile-context/spec.md) — uses `_parseSpecId` to split canonical spec IDs
- [`core:workspace`](../workspace/spec.md) — workspace-qualified spec ID format definition
