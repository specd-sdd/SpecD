...
2. **Resolve extends chain** — if the base schema declares `extends`, resolve the parent schema recursively until a schema with no `extends` is reached. Each resolved schema becomes an `extends` layer. Detect cycles by tracking resolved file paths; throw `SchemaValidationError` if a cycle is found. When cascading metadata across the chain (`overlayData`), `compat` SHALL cascade such that a child schema inherits its parent's `compat` unless the child explicitly overrides it.
