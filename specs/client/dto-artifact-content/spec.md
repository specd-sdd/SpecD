...
Client-side type for **Dto Artifact Content**, kept in parity with the matching `api:dto-*` spec so remote and embedded Studio render the same JSON the API emits.
...
- [`api:dto-artifact-content`](../../api/dto-artifact-content/spec.md) — mirror API DTO
