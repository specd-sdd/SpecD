...
Client-side type for **Dto Workspace Spec Tree**, kept in parity with the matching `api:dto-*` spec so remote and embedded Studio render the same JSON the API emits.
...
- [`api:dto-workspace-spec-tree`](../../api/dto-workspace-spec-tree/spec.md) — mirror API DTO
