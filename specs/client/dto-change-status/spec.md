...
Client-side type for **Dto Change Status**, kept in parity with the matching `api:dto-*` spec so remote and embedded Studio render the same JSON the API emits.
...
- [`api:dto-change-status`](../../api/dto-change-status/spec.md) — mirror API DTO
