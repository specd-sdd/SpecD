...
Client-side type for **Dto Compiled Context**, kept in parity with the matching `api:dto-*` spec so remote and embedded Studio render the same JSON the API emits.
...
- [`api:dto-compiled-context`](../../api/dto-compiled-context/spec.md) — mirror API DTO
