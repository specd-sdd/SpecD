# Verification: Handler Specs Mutate
...
- **THEN** every handler path matches `api:routes-specs-mutate` under `/v1`
...
#### Scenario: Handler invokes ValidateSpecs

- **WHEN** a valid request for this handler is processed
- **THEN** `ValidateSpecs` runs inside @specd/core
- **AND** handler does not reimplement lifecycle or validation rules

#### Scenario: Handler invokes saveMetadata

- **WHEN** a valid request for this handler is processed
- **THEN** `saveMetadata` runs inside @specd/core
- **AND** handler does not reimplement lifecycle or validation rules

#### Scenario: Handler invokes generateMetadata

- **WHEN** a valid request for this handler is processed
- **THEN** `generateMetadata` runs inside @specd/core
- **AND** handler does not reimplement lifecycle or validation rules
