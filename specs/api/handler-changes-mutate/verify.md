# Verification: Handler Changes Mutate
...
- **THEN** every handler path matches `api:routes-changes-mutate` under `/v1`
...
#### Scenario: Handler invokes SaveChangeArtifact

- **WHEN** a valid request for this handler is processed
- **THEN** `SaveChangeArtifact` runs inside @specd/core
- **AND** handler does not reimplement lifecycle or validation rules

#### Scenario: Handler invokes ValidateArtifacts

- **WHEN** a valid request for this handler is processed
- **THEN** `ValidateArtifacts` runs inside @specd/core
- **AND** handler does not reimplement lifecycle or validation rules

#### Scenario: Handler invokes TransitionChange

- **WHEN** a valid request for this handler is processed
- **THEN** `TransitionChange` runs inside @specd/core
- **AND** handler does not reimplement lifecycle or validation rules
...
- **WHEN** inspecting `handler-changes-mutate` module imports
