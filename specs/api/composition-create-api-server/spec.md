...
The factory MUST call `createKernel` once per process with a `LogRingBuffer` (callback destination) and build per-request context via `createApiContext`.
...
- [`core:kernel`](../../core/kernel/spec.md) — kernel
