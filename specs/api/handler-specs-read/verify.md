# Verification: Handler Specs Read
...
- **THEN** every handler path matches `api:routes-specs-read` under `/v1`
...
#### Scenario: Handler invokes spec repository get (metadata)

- **WHEN** a valid request for this handler is processed
- **THEN** spec repository get (metadata) runs inside @specd/core
- **AND** handler does not reimplement lifecycle or validation rules

#### Scenario: Handler invokes GetOutline

- **WHEN** a valid request for this handler is processed
- **THEN** `GetOutline` runs inside @specd/core
- **AND** handler does not reimplement lifecycle or validation rules
...
- **WHEN** inspecting `handler-specs-read` module imports
