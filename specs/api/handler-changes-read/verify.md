# Verification: Handler Changes Read
...
- **THEN** every handler path matches `api:routes-changes-read` under `/v1`
...
#### Scenario: Handler invokes GetStatus (with optional ifModifiedSince)

- **WHEN** a valid request for this handler is processed
- **THEN** `GetStatus` (with optional `ifModifiedSince`) runs inside @specd/core
- **AND** handler does not reimplement lifecycle or validation rules

#### Scenario: Handler invokes GetChangeArtifact
...
- **WHEN** a valid request for this handler is processed
- **THEN** `CompileContext` runs inside @specd/core
- **AND** handler does not reimplement lifecycle or validation rules
...
- **WHEN** inspecting `handler-changes-read` module imports
