# Verification: Handler Changes Collection
...
- **THEN** every handler path matches `api:routes-changes-collection` under `/v1`
...
#### Scenario: Handler invokes CreateChange

- **WHEN** a valid request for this handler is processed
- **THEN** `CreateChange` runs inside @specd/core
- **AND** handler does not reimplement lifecycle or validation rules

#### Scenario: Handler invokes kernel.changes.list

- **WHEN** a valid request for this handler is processed
- **THEN** `kernel.changes.list` runs inside @specd/core
- **AND** handler does not reimplement lifecycle or validation rules

#### Scenario: Handler invokes kernel.changes.listDrafts
...
- **WHEN** inspecting `handler-changes-collection` module imports
