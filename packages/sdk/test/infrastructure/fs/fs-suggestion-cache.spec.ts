  ...
  it('performs self-validating get, set, and findSpecByFile', async () => {
    const mockRepo = {
      get: vi.fn().mockResolvedValue({
        workspace: 'cli',
        path: 'spec-deps',
        artifacts: [{ filename: 'spec.md', lastModified: '2026-08-17T12:00:00Z', hash: 'hash-abc' }],
      }),
    } as unknown as SpecRepository

    const mockGraph = {
      getGraphHealth: vi.fn().mockResolvedValue({ currentRef: 'fingerprint-1', freshness: '2026-08-17T12:00:00Z' }),
    ...
    mockRepo.get = vi.fn().mockResolvedValue({
      workspace: 'cli',
      path: 'spec-deps',
      artifacts: [{ filename: 'spec.md', lastModified: '2026-08-17T13:00:00Z', hash: 'hash-changed' }],
    })

    expect(await cache.get('cli:spec-deps')).toBeNull()

    // Reverse lookup code -> spec
    expect(await cache.findSpecByFile('cli:packages/cli/src/commands/spec/deps.ts')).toBe('cli:spec-deps')
    expect(await cache.findSpecByFile('packages/cli/src/commands/spec/deps.ts')).toBe('cli:spec-deps')
...
describe('FsSpecDepsSuggestionCache', () => {
  ...
  it('performs self-validating get, set, flush and invalidate', async () => {
    ...
    await cache.set('cli:spec-deps', {
      ...
      suggestedDependsOn: [{ specId: 'cli:host-context', title: 'HostContext', reason: 'code import' }],
    })
