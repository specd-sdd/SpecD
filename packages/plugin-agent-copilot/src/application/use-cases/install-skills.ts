...
/**
 * Installs selected skills into Copilot's project-local skills directory.
 */
export class InstallSkills {
  /**
   * Installs one or more skills for Copilot.
   *
...
function renderFrontmatter(frontmatter: Frontmatter, content: string): string {
  ...
  appendYamlField(lines, 'user-invocable', frontmatter['user-invocable'])
  appendYamlField(lines, 'disable-model-invocation', frontmatter['disable-model-invocation'])
  lines.push('---', '', content)
  return `${lines.join('\n')}\n`
}
