...
const repositoryMock = {
  ...
  listSharedFiles: vi.fn(async () => [{ filename: 'shared.md', content: 'shared-content' }]),
}
...
describe('plugin-agent-claude create()', () => {
  ...
  it('given an agent, when install is called, then generates Claude-specific YAML frontmatter', async () => {
    const projectRoot = await createTempProjectRoot()
    const config = makeMockConfig(projectRoot)
        ...
        expect.objectContaining({
          variables: expect.objectContaining({
            frontmatter: {
              ...
              tools: 'Bash(node *), Bash(specd *), Bash(cat *), Bash(rm *), Read, Write',
            },
          }),
        }),
      )
