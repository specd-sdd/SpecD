...
async function readManifest(): Promise<{ name: string; version: string }> {
  ...
  throw new PluginValidationError('@specd/plugin-agent-codex', ['specd-plugin.json'])
}

/**
 * Creates a Codex agent plugin instance.
 *
 * @returns Configured CodexAgentPlugin instance.
 */
