...
function mockBatchSchema(
  ...
  return { raw: false as const, schema: new Schema('schema', 'test-schema', 1, artifacts, []) }
}

function mockBatchStep(
  spec: string | null,
  artifact: string,
  passed = true,
  failures: Array<{ artifactId: string; description: string }> = [],
) {
  return { spec, artifact, passed, failures, warnings: [], files: [] }
}
...
describe('change validate', () => {
  ...
  it('--all validates all specIds in the change', async () => {
    ...
    kernel.changes.validateBatch.execute.mockResolvedValue({
      passed: true,
      results: [
        mockBatchStep('default:auth/login', 'specs'),
        mockBatchStep('default:auth/logout', 'specs'),
      ],
    })
    ...
    expect(out).toContain('validation is structural')
    expect(out).toContain('validated 2/2 steps')
  })
  ...
  it('--all with partial failures exits 1', async () => {
    ...
    kernel.changes.validateBatch.execute.mockResolvedValue({
      ...
      results: [
        mockBatchStep('default:auth/login', 'specs'),
        mockBatchStep('default:auth/logout', 'specs', false, [
          { artifactId: 'specs', description: 'missing delta' },
        ]),
      ],
    })
    ...
    const out = stdout()
    expect(out).toContain('validated feat/default:auth/login [artifact:specs]: all artifacts pass')
    expect(out).toContain('validation failed feat/default:auth/logout [artifact:specs]')
    expect(out).toContain('validated 1/2 steps')
    expect(process.exitCode).toBe(1)
  })
  ...
  it('--all passes --artifact to each spec', async () => {
    ...
    kernel.changes.validateBatch.execute.mockResolvedValue({
      passed: true,
      results: [
        mockBatchStep('default:auth/login', 'proposal'),
        mockBatchStep('default:auth/logout', 'proposal'),
      ],
    })
  ...
  it('--all invokes change-scoped steps without specPath', async () => {
    ...
    kernel.changes.validateBatch.execute.mockResolvedValue({
      passed: true,
      results: [mockBatchStep(null, 'proposal'), mockBatchStep('default:auth/login', 'specs')],
    })
  ...
  it('--all JSON output has batch structure', async () => {
    ...
    kernel.changes.validateBatch.execute.mockResolvedValue({
      passed: true,
      results: [mockBatchStep('default:auth/login', 'specs')],
    })
    ...
    expect(parsed.results[0].artifact).toBe('specs')
    expect(parsed.results[0].passed).toBe(true)
    expect(Array.isArray(parsed.results[0].files)).toBe(true)
    expect(Array.isArray(parsed.results[0].warnings)).toBe(true)
